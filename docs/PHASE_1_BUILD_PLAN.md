...
**Repository:** servicenow-flowguard
...
Build a production-ready ServiceNow scoped application that cross-environment flow validation and migration safety tool that detects flow differences, schema drift, and breaking changes between servicenow sandbox and production instances.. The application must be installable on any Australia-era ServiceNow instance and must pass automated functional tests with zero external runtime dependencies.
